...
def step_populate_contact_form(
    contact_page: ContactPage, datatable: dict[str, str]
) -> None:
...
@then(
    parsers.parse("I should see that the error message '{error_message}' is displayed")
)
def step_validate_field_error_message(
    contact_page: ContactPage, error_message: str
) -> None:
...
def step_field_error_message_not_visible(
    contact_page: ContactPage, error_message: str
) -> None:
